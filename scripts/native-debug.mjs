...
if (mode === 'geometry') code = `(()=>{const v=app.workspace.getLeavesOfType('knowledge-atlas-view')[0].view;return JSON.stringify({root:v.contentEl.getBoundingClientRect().toJSON(),stage:v.stage.getBoundingClientRect().toJSON(),graphWidth:v.graph.width(),graphHeight:v.graph.height(),zoom:v.graph.zoom(),bbox:v.graph.getGraphBbox(),selected:v.selected,closed:v.closed,nodes:v.visible.map(n=>({id:n.id,x:n.x,y:n.y}))})})()`;
if (mode === 'rerun') code = `(async()=>{ if(app.vault.getName() !== 'Atlas-test-vault') throw Error('Wrong vault'); await app.plugins.disablePlugin('atlas-native-tests'); await app.plugins.disablePlugin('knowledge-atlas'); await app.plugins.enablePlugin('knowledge-atlas'); await app.plugins.enablePlugin('atlas-native-tests'); return 'Test plugins reloaded'; })()`;
if (mode === 'screenshot') {
